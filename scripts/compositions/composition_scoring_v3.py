...
def _run_joint_steered_composition(
    ...
    if mode in ("generate", "full"):
        ...
        _generate_completions_csv(
            ...
            log_path=log_path,
            header=(
                f"pair={trait_a}+{trait_b}  joint normalize=per_axis α={alpha}  "
...
def main() -> None:
    ...
    n_pairs = len(pairs)
    print(f"\nEvaluating {n_pairs} composition pairs at α={COMPOSITION_ALPHA}, normalize=per_axis")
    print("=" * 72)
    if mode == "generate":
        print(
    ...
    for i, (a, b) in enumerate(pairs, 1):
        ...
        base_log = LOGS_DIR / f"composition_v3_{a}__{b}_baseline.log"
        ...
        single_a_log = LOGS_DIR / f"composition_v3_{a}__{b}_single_a_alpha{COMPOSITION_ALPHA}.log"
        ...
        single_b_log = LOGS_DIR / f"composition_v3_{a}__{b}_single_b_alpha{COMPOSITION_ALPHA}.log"
        ...
        steer_log = LOGS_DIR / f"composition_v3_{a}__{b}_alpha{COMPOSITION_ALPHA}.log"
        ...
        if df_traj is not None:
            ...
            print(
                ...
                f"pred(per_axis)={sanity['pred_diff_per_axis']:+.3f}  "
    ...
    if mode == "generate":
        print(
            ...
            "scripts.compositions.composition_scoring_v3` from a host with internet "
