...
def write_outputs(tables, gate_removals, loo, verif):
    ...
    lines.append("\n## Figures\n")
    lines.append("Generated by `analysis/rq1_mrqap_plots.py` into `analysis/figures/mrqap/`.\n")
    figs = [
        ("fig_mrqap_matrices",
         "**The data the test runs on.** 8×8 symmetric trait-dyad matrices, one row per scheme. "
         "`COS` = signed cosine between the two unit steering directions (red = aligned, blue = "
         "opposed). `Smat` = directional outcome (red = joint expression *exceeds* the individual "
         "injections → reinforcement; blue = suppression). `Mmat` = joint-expression magnitude. "
         "Grey diagonal is ignored; **× marks dyads removed by the coherence gate** (coh<30) and "
         "dropped from every matrix for that scheme. normalized_sum keeps all 28 dyads; per_axis "
         "loses 6, and they concentrate on *humorous* and *hallucinating* — the low-coherence, "
         "mostly-opposed side. The upper triangles of these matrices are the edge vectors the "
         "permutation test operates on."),
        ("fig_mrqap_direction_scatter",
         "**Headline directional test + sign check.** Each point is one trait pair (28 for "
         "normalized_sum, 22 for per_axis); y = `Smat` (above the dashed line = reinforcement, "
         "below = suppression). *Left column, vs signed cosine*: clear positive slope — aligned "
         "directions reinforce, opposed directions suppress (β|STR = +0.54, exact p = 0.009 for "
         "normalized_sum; +0.52, p = 0.051 for per_axis). *Right column, vs |cosine|*: flat "
         "(r = +0.02, p = 0.92) — the size of the cosine predicts nothing. Conclusion: the **sign** "
         "of the cosine carries the effect, not its magnitude. Titles report the raw Mantel r and "
         "the STR-controlled partial slope, each with its exact permutation p."),
        ("fig_mrqap_perm_null",
         "**What the exact test actually computes.** Histogram = the standardized partial slope "
         "(β_cos | STR) over all 40,320 relabelings of the 8 trait nodes — the null distribution "
         "that respects dyadic dependence. Black line = observed slope; coloured bars = the "
         "two-sided tail |β| ≥ |β_obs|; that tail fraction *is* the exact p. normalized_sum sits "
         "far in the right tail (p = 0.009); per_axis sits right at the 5% edge (p = 0.051). This "
         "replaces the invalid test that shuffled cosine labels across the 76 pooled pair×scheme "
         "rows."),
        ("fig_mrqap_magnitude_scatter",
         "**Magnitude sanity check — expected null, but it isn't.** `Mmat` (joint-expression "
         "magnitude) vs signed cosine. Both schemes show a strong positive slope (r ≈ 0.65, "
         "p = 0.002 for normalized_sum; partial β|STR = +0.51, p = 0.015). Joint magnitude also "
         "grows with alignment even after controlling individual strength — so cosine is **not** "
         "orthogonal to magnitude. Reported as-is rather than treated as a clean control."),
    ]
    for name, cap in figs:
        lines.append(f"\n![{name}](../figures/mrqap/{name}.png)\n")
        lines.append(f"*{cap}*\n")
    ...
    lines.append("***Single-trait leverage.** Each row leaves one trait out (7 nodes, 5040 perms) "
                 "and refits the headline `Smat ~ COS | STR`; the diamond is the full 8-trait "
                 "estimate, the dashed line its β. Filled = p<0.05, open = p≥0.05. normalized_sum "
                 "stays significant in 7 of 8 folds and collapses only when *formality* is removed "
                 "(β → 0.25, p = 0.32) — formality carries much of the suppression signal. per_axis "
                 "is fragile: most folds land above 0.05. Quantifies how much the effect leans on "
                 "individual traits.*\n")
