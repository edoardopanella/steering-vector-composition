...
and reports the three filters, in order:
  ...
  have 28. It was dropped a priori at the Phase 12.5 design commit (E15.5)
  because the vector is non-expressing: its single-trait delta is negative
  (steering toward power_seeking does not even produce power_seeking — Phase 2/3
  "context-conditioning failure", E15.3 "goes negative at alpha=3, confirms
  broken trait"). Every pair containing it therefore has a structurally dead
  ...
  cosine. See paper/experiments_log.md Phases 10-15.16 (E14 Problem 2, E15.1).
...
Output (results regenerated each run; `## Conclusions` preserved):
    ...
    analysis/results/RQ1/rq1_filtering/rq1_filtering.{md,json}
"""
...
OUT_DIR = REPO_ROOT / "analysis/results/RQ1/rq1_filtering"
...
def _render_results(r: dict) -> str:
    ...
    P += [
        ...
        "",
        f"_Auto-generated by `analysis/notebooks_v2/{OUT_STEM}.py` on "
        ...
        "",
        f"`{EXCLUDED_BEHAVIOUR}` was dropped a priori at the Phase 12.5 design "
        "commit (experiments_log E15.5). The reason is that the vector is "
        ...
        "to elicit its own behaviour (Phase 2/3 context-conditioning failure; "
        "E15.3 \"goes negative at α=3 — confirms broken trait\"). Its partner "
        ...
        "deletes the broken generations, so a naïve average on the gated file "
